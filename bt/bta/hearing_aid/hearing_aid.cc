...
#include "embdrv/rel/include/functions.h"
#include "gap_api.h"
#include "gatt_api.h"
...
FILE  *fid_decoder;
FILE  *before_encode;
/* C:\Ubuntu_shared\data.raw (5/20/2019 1:04:50 PM)
   StartOffset(h): 00000000, EndOffset(h): 0000013F, Length(h): 00000140 */

/* C:\Ubuntu_shared\data.raw (5/20/2019 2:35:08 PM)
   StartOffset(h): 00000000, EndOffset(h): 0000013F, Length(h): 00000140 */
...
int       sampleRate = 16000, nChannels = 1, bipsIn = 16, nSamples = 0,\
		 	 	 	 nBytes = 0 ,desired_bit_rate = 64000;
int       nSamplesRead = 0, real_bitrate = 0, frame = 1, delay = 0;
int       encoder_size = 0, decoder_size = 0, scratch_size = 0;
uint8_t     scratch[LC3_DEC_MAX_SIZE];


LC3_Enc  encoder_left[LC3_ENC_MAX_SIZE] ;
LC3_Dec  decoder_left[LC3_DEC_MAX_SIZE] ;
...
//      /* Setup Encoder */
      err        = lc3_enc_init(encoder_left, sampleRate, nChannels, desired_bit_rate);
		LOG(INFO) << "err of enc = " << (int) err;

      err          = lc3_dec_init(decoder_left, sampleRate, nChannels, LC3_PLC_STANDARD);
		LOG(INFO) << "err of dec = " << (int) err;

        nSamples     = lc3_enc_get_input_samples(encoder_left);
        real_bitrate = lc3_enc_get_real_bitrate(encoder_left);
        nBytes = lc3_enc_get_num_bytes(encoder_left);
        memset(scratch, 0xcb, LC3_DEC_MAX_SIZE);
		LOG(INFO) << "nSamples= " << (int) nSamples;
		LOG(INFO) << "real_bitrate= " << (int) real_bitrate;
		LOG(INFO) << "nBytes= " << (int) nBytes;
...
//      encoder_state_right = g722_encode_init(nullptr, 64000, G722_PACKED);
  ...
  void scale_8_to_16(const uint8_t *in, int16_t *out, int n)
  {
      int i;
      for (i = 0; i < n/2; i++)
      {
...
//      LOG(FATAL) << "num_samples is not even: " << num_samples;
    ...
    if (left) {
      ...
      // buffer size
      encoded_data_left.resize(LC3_MAX_BYTES);
      LOG(INFO) << "chan_left.size() "<<chan_left.size() ;

      int16_t *input_samples[] = {(int16_t *)data.data(),(int16_t *)(data.data()+ 320)};


      err = lc3_enc16(encoder_left, &input_samples[0],  encoded_data_left.data(), &nBytes,(void*)scratch);
      LOG(INFO) << "err of encoding "<<err ;

	      err = lc3_enc16(encoder_left, &input_samples[1],  (void*)(encoded_data_left.data() + 80), &nBytes,(void*)scratch);
	      LOG(INFO) << "err of encoding "<<err ;
	  int encoded_size = nBytes*2;
