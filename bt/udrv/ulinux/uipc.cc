...
#define SAFE_FD_ISSET(fd, set) (((fd) == -1) ? false : FD_ISSET((fd), (set))) //FD_ISSET((fd), (set))
...
//  LOG(INFO) << "MAIN INIT " <<uipc.signal_fds[0];
...
static int uipc_check_fd_locked(tUIPC_STATE& uipc, tUIPC_CH_ID ch_id) {
  ...
  // ch_id);
  LOG(INFO) << __func__ << "chan id = " << (int) ch_id;
//  LOG(INFO) <<  uipc.read_set.fds_bits[0] ;
//  LOG(INFO) <<  uipc.active_set.fds_bits[0] ;
//  LOG(INFO) << "SERVER chan srv FD :" << (int) uipc.ch[ch_id].srvfd ;
//  LOG(INFO) << "SERVER FD :" << (int) uipc.ch[ch_id].fd ;

  if (SAFE_FD_ISSET(uipc.ch[ch_id].srvfd, &uipc.read_set)) {
    LOG(INFO) << "INCOMING CONNECTION ON CH " << (int)ch_id;

    // Close the previous connection
    if (uipc.ch[ch_id].fd != UIPC_DISCONNECTED) {
      LOG(ERROR) <<"CLOSE CONNECTION (FD %d)", uipc.ch[ch_id].fd;
    ...
    LOG(INFO) << "NEW FD %d" << uipc.ch[ch_id].fd;
    ...
    if ((uipc.ch[ch_id].fd >= 0) && uipc.ch[ch_id].cback) {
      ...
      LOG(INFO) <<"ADD FD %d TO ACTIVE SET" << uipc.ch[ch_id].fd;
      FD_SET(uipc.ch[ch_id].fd, &uipc.active_set);
      uipc.max_fd = MAX(uipc.max_fd, uipc.ch[ch_id].fd);
    }

    if (uipc.ch[ch_id].fd < 0) {
    	LOG(ERROR) <<"FAILED TO ACCEPT CH %d"<< ch_id;
      return -1;
    }
//    LOG(INFO) << "UIPC_OPEN_EVT CHECKS";
...
//  LOG(INFO) << "DUMMY DATA" ;
  ...
  if (SAFE_FD_ISSET(uipc.ch[ch_id].fd, &uipc.read_set)) {
    // BTIF_TRACE_EVENT("INCOMING DATA ON CH %d", ch_id);
	    LOG(INFO) << "UIPC_RX_DATA_READY_EVT CHECKS";
...
static int uipc_setup_server_locked(tUIPC_STATE& uipc, tUIPC_CH_ID ch_id,
  ...
  LOG(INFO) << "SETUP CHANNEL SERVER  "<< (int)ch_id;
  ...
  LOG(INFO)<< "ADD SERVER FD TO ACTIVE SET "<< fd ;
...
static int uipc_close_ch_locked(tUIPC_STATE& uipc, tUIPC_CH_ID ch_id) {
  ...
  LOG(INFO) << "UIPC_CLOSE_EVT CHECKS" ;
...
//    LOG(INFO) << __func__ ;
//    LOG(INFO) << "read set " << uipc.read_set.fds_bits[0] ;
//    LOG(INFO) << "active set " << uipc.active_set.fds_bits[0] ;
//    LOG(INFO) << "max fd  set " << uipc.max_fd ;

    LOG(INFO) << __func__ << "thread id"<< (int)pthread_self();
    result = select(uipc.max_fd + 1, &uipc.read_set, NULL, NULL, NULL);
//    LOG(INFO) << "FOUND A SOCKET";
    if (result == 0) {
...
//  LOG(INFO )<<"UIPC READ THREAD EXITING";

  uipc_main_cleanup(uipc);

  uipc.tid = 0;

//  LOG(INFO)<<"UIPC READ THREAD DONE";
...
void uipc_stop_main_server_thread(tUIPC_STATE& uipc) {
  /* request shutdown of read thread */
	LOG(ERROR) << "stop main server thread ";
  {
...
//	  BTIF_TRACE_DEBUG("UIPC_Open : ch_id %d, p_cback %x", ch_id, p_cback);
	 LOG(INFO)<<"UIPC_Open : ch_id " <<(int) ch_id << "p_cback " << p_cback;

  std::lock_guard<std::recursive_mutex> lock(uipc.mutex);

  if (ch_id >= UIPC_CH_NUM) {
	  LOG(INFO) <<"RETURN FLASE ...";
    return false;
  }

  if (uipc.ch[ch_id].srvfd != UIPC_DISCONNECTED) {
    LOG(ERROR) << "CHANNEL %d ALREADY OPEN"<<(int) ch_id;
    return 0;
  }
...
bool UIPC_Send(tUIPC_STATE& uipc, tUIPC_CH_ID ch_id,
  ...
  LOG(INFO) << "WRITE UIPC CHAN " <<(int) ch_id;
  std::lock_guard<std::recursive_mutex> lock(uipc.mutex);
