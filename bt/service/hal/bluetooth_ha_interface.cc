...
#include "service/hal/bluetooth_ha_interface.h"
...
class BluetoothHaInterfaceImpl : public BluetoothHaInterface {
 public:
	BluetoothHaInterfaceImpl() = default;
  ~BluetoothHaInterfaceImpl() override {
//    A2dpSinkDisable();
//    A2dpSourceDisable();
  }
...
//  }
  ...
  bool HaEnable() override {
    if (ha_enabled_) {
      return true;
    }
    //TODO IMPLEMENT CALL BACKS INSTEAD OF  null ptr
    if (hal_HA_iface_->init(nullptr) != BT_STATUS_SUCCESS) {
      LOG(ERROR) << "Failed to initialize HA interface";
      return false;
    }
    ha_enabled_ = true;
    return true;
...
//  }
  ...
  virtual const hearing_aid::HearingAidInterface* GetHaHALInterface() override {
    return hal_HA_iface_;
  }
  ...
  bool Initialize() {
    ...
    const auto* hal_source_iface =
        reinterpret_cast<const  hearing_aid::HearingAidInterface*>(
            bt_iface->get_profile_interface(BT_PROFILE_HEARING_AID_ID));
    if (!hal_source_iface) {
      LOG(ERROR) << "Failed to obtain HA source interface handle";
      return false;
    }


    hal_HA_iface_ = hal_source_iface;
...
//  base::ObserverList<A2dpSinkObserver> a2dp_sink_observers_;
  ...
  const hearing_aid::HearingAidInterface* hal_HA_iface_ = nullptr;

  bool ha_enabled_ = false;

  DISALLOW_COPY_AND_ASSIGN(BluetoothHaInterfaceImpl);
};
